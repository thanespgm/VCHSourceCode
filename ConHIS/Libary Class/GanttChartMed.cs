...
namespace ConHIS
{
    public class GanttChartMed : UserControl
    {
        ...
        public bool AllowEditBarWithMouse { get; set; } = false;
        public bool AllowManualEditBar { get; set; } = false;
        public bool AllowChange { get; set; } = false;
        public Font TimeFont { get; set; } = new Font("Athiti", emSize: 7, FontStyle.Regular);
        public DateTime FromDate { get; set; }
        public DateTime ToDate { get; set; }
        public string MouseOverRowText { get; set; } = "";
        public Object MouseOverRowValue { get; set; } = null;
        public DateTime MouseOverColumnDate { get; set; }
        public Pen GridColor { get; set; } = Pens.Silver;
        public Font RowFont { get; set; } = new Font("Athiti", 9.75F, FontStyle.Regular);
        public Font DateFont { get; set; } = new Font("Athiti", 9, FontStyle.Regular);
        ...
        private int barHeight = 15;       // ความกว้างระหว่างบรรทัด
        ...
        public GanttChartMed()
        {
            ToolTip.AutoPopDelay = 30000;
            ToolTip.InitialDelay = 1000;
            ToolTip.OwnerDraw = true;

            objBmp = new Bitmap(Screen.PrimaryScreen.Bounds.Width, Screen.PrimaryScreen.Bounds.Height);
        ...
        private void SetBarStartLeft(string rowText)
        {
            ...
            int length = (int)gfx.MeasureString(rowText, RowFont, 500).Width;
        ...
        private void DrawHeader(Graphics gfx, List<Header> headerList)
        {
            ...
            foreach (Header header in headerList)
            {
                ...
                if (showDateHeader == true)
                {
                    ...
                    {
                        string strC = header.HeaderTextInsteadOfTime;
                        if (strC != str)
                        {
                            str = strC;
                            gfx.DrawString(strC, DateFont, Brushes.Navy, startPos, 0);
                        }
                    }
                    else
                    {
                        str = header.Time.ToString("d MMMM yyyy");
                        gfx.DrawString(str, DateFont, Brushes.Navy, startPos, 0);
                    }

                    // gfx.DrawString(str, dateTextFont, Brushes.Navy, startPos, 0);
                }

                // Show time

                gfx.DrawString(header.HeaderText, TimeFont, Brushes.Navy, startPos, headerTimeStartTop);
                index += 1;
        ...
        private void DrawBars(Graphics grfx, bool ignoreScrollAndMousePosition = false)
        {
            ...
            foreach (ChartBarDate bar in bars)
            {
                ...
                if (MouseOverRowText == bar.Text & bar.StartValue <= MouseOverColumnDate & bar.EndValue >= MouseOverColumnDate)
                {
                ...
                if (bar.StartValue != null & endValue != null)
                {
                    if ((index >= scrollPos & index < barsViewable + scrollPos) | ignoreScrollAndMousePosition == true)
                    {
                        ...
                        grfx.DrawRectangle(Pens.Silver, obRect);
                        grfx.FillRectangle(obBrush, obRect);

                        // Draws the rowtext

                        grfx.DrawString(bar.Text, RowFont, Brushes.Teal, 0, barStartTop + (barHeight * (index - scrollPos)) + (barSpace * (index - scrollPos)));
        ...
        public void DrawNetVertical(Graphics grfx)
        {
            ...
            foreach (Header header in shownHeaderList)
            {
                ...
                grfx.DrawLine(Pens.Silver, barStartLeft + (index * widthPerItem), headerLocationY, barStartLeft + (index * widthPerItem), lastLineStop);
                index += 1;

                lastHeader = header;
            }

            grfx.DrawLine(GridColor, barStartLeft + (index * widthPerItem), headerTimeStartTop, barStartLeft + (index * widthPerItem), lastLineStop);
        }
        ...
        public void DrawNetHorizontal(Graphics grfx)
        {
            ...
            for (index = 0; index <= GetIndexChartBar("QQQQQQ"); index++)
            {
                foreach (ChartBarDate bar in bars)
                {
                    grfx.DrawLine(GridColor, 0, barStartTop + (barHeight * index) + (barSpace * index), width, barStartTop + (barHeight * index) + (barSpace * index));
                }
            }
        ...
        #endregion Draw the Gantt Chart
        ...
        private List<Header> GetFullHeaderList()
        {
            ...
            if (interval.TotalDays < 1)     // ถ้าระยะเวลาเริ่มต้นจนถึงสิ้นสุดไม่เกิน 1 วัน
            {
                ToDate = FromDate.AddHours(23); // 24 Hours

                DateTime tmpTime = newFromTime;
                newFromTime = tmpTime.AddHours(FromDate.Hour);

                if (FromDate.Minute < 59 & FromDate.Minute > 29)
                {
                    tmpTime = newFromTime;
                ...
                while (newFromTime <= ToDate)
                {
                    ...
                    header.HeaderTextInsteadOfTime = newFromTime.ToString("dd MMMM yyyy");
                    ...
                    newFromTime = newFromTime.AddHours(1);

                    // The minimum interval of time between the headers
                }
            }
            else if (interval.TotalDays > 1 && interval.TotalDays <= 2)     // ถ้าระยะเวลาเริ่มต้นจนถึงสิ้นสุดไม่เกิน 2 วัน
            {
                ToDate = FromDate.AddHours(32); // 24 Hours
            ...
            else if (interval.TotalDays < 60) // ถ้าระยะเวลาเริ่มต้นจนถึงสิ้นสุดไม่เกิน 2 เดือน
            {
                while (newFromTime <= ToDate)
                {
                    Header header = new Header();
                    ...
                    header.HeaderText = "";
                    header.HeaderTextInsteadOfTime = newFromTime.ToString("dd MMM"); ;
            ...
            else if (interval.TotalDays < 90)  // ถ้าระยะเวลาเริ่มต้นจนถึงสิ้นสุดไม่เกิน 3 เดือน
            {
                while (newFromTime <= ToDate)
                {
                    Header header = new Header();
            ...
            else  // ถ้าระยะเวลาเริ่มต้นจนถึงสิ้นสุดเกิน 3 เดือน
            {
                while (newFromTime <= ToDate)
                {
                    Header header = new Header();
        ...
        public void GanttChart_MouseMove(object sender, MouseEventArgs e)
        {
                ...
                if (AllowManualEditBar == true)
                {
                    if (barIsChanging >= 0)
                    {
                        if (AllowChange)
                        {
                            ...
                            DateTime tempValue = MouseOverColumnDate;
            ...
            else
            {
                ...
                if ((MouseOverRowValue == null & (rowValue != null)) | ((MouseOverRowValue != null) & rowValue == null) | scrollBarStatusChanged == true)
                {
                    PaintChart();
                }
            }
            ...
            if (AllowManualEditBar == true)
            {
                if (mouseHoverBarIndex > -1)
                {
                    if (mouseHoverPart == MouseOverPart.BarLeftSide)
                    {
                        ...
                        bars[mouseHoverBarIndex].StartValue = MouseOverColumnDate;
                        PaintChart();
                    }
                    else if (mouseHoverPart == MouseOverPart.BarRightSide)
                    {
                        barIsChanging = mouseHoverBarIndex;
                        bars[mouseHoverBarIndex].EndValue = MouseOverColumnDate;
                        PaintChart();
                    }
                }
            }
        ...
        public void ToolTipText_Draw(Object sender, DrawToolTipEventArgs e)
        {
            ...
            using (Font font = new Font("Athiti", 12, FontStyle.Regular))
            {
                ...
                e.Graphics.DrawString(text, font, Brushes.Navy, x, y);
            }
            ...
            {
                Font font = new Font("Athiti", 10, FontStyle.Regular);
                newStr = str;
                ...
                using (font)
                {
                    ...
                    e.Graphics.DrawString(str, font, Brushes.DimGray, x, y);
                }
        ...
        public void ToolTipText_Popup(Object sender, PopupEventArgs e)
        {
            ...
            e.ToolTipSize = new Size(500, height);
        }
